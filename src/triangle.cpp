...
static void reportIncomplete(
  ...
  std::cout << "  incomplete     nothing was drawn because student.cpp is not "
               "finished:\n";
  if (TRIANGLE.empty())
  ...
  if (attrs.size() != 2)
    std::cout << "                 TASK 1c: " << attrs.size()
              << " of 2 attributes described\n";
}

static void reportCentroid(const std::vector<std::uint8_t>& rgba,
                           const Target& target) {
  glm::vec2 centroid(0.0f);
  for (const Vertex& v : TRIANGLE) centroid += v.pos;
  centroid /= static_cast<float>(TRIANGLE.size());

  std::uint32_t x =
      static_cast<std::uint32_t>((centroid.x * 0.5f + 0.5f) * target.width);
  std::uint32_t y =
      static_cast<std::uint32_t>((centroid.y * 0.5f + 0.5f) * target.height);
...
int main() {
  try {
    ...
    Buffer uniform =
        createBuffer(dev, params.size(), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    uploadBuffer(uniform, params.data(), params.size());

    Buffer vertices =
        createBuffer(dev, sizeof(Vertex) * std::max(vertexCount, 1u),
    ...
    if (ready) {
      ...
      pipeline = createPipeline(dev, renderPass, layout, vertShader, fragShader,
                                target);
    }

    Timing timing =
        timePass(dev, WARMUP_FRAMES, MEASURED_FRAMES, [&](VkCommandBuffer cmd) {
          VkClearValue clear{};
          ...
          if (pipeline != VK_NULL_HANDLE) {
            ...
            vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                    layout, 0, 1, &descs.set, 0, nullptr);
    ...
    double diff =
        diffFromReference("triangle", rgba, target.width, target.height);

    std::cout << std::fixed << std::setprecision(1);
    std::cout << "  vertex buffer  " << vertexCount << " vertices, stride "
              << binding.stride << ", offsets [";
    ...
    if (diff < 0.0 && ready && drawn > 0.0) {
      writePng(std::string(REFERENCE_DIR) + "/triangle.png", rgba, target.width,
               target.height);
      std::cout << "  image          no reference found, wrote one: check it "
                   "by eye\n";
    } else if (diff < 0.0) {
    ...
    if (!ready) {
      ...
      std::cout << "  hint           your uniform block is all zeros, so every "
                   "vertex\n"
                << "                 collapses to the origin. TASK 2 writes "
                   "that matrix\n";
    } else if (drawn == 0.0) {
      std::cout << "  hint           nothing was rasterized. Check your "
                   "winding against\n"
                << "                 VK_FRONT_FACE_COUNTER_CLOCKWISE, and the "
                   "sign of y\n";
    }

    std::cout << std::setprecision(3);
    std::cout << "  gpu time       " << timing.medianMs << " ms median, "
              << timing.p95Ms << " ms p95" << std::endl;

    showWindow(win);
    while (windowOpen(win)) presentTarget(dev, win, target);
